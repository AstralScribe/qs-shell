...
QString FileSystemEntry::path() const { return m_path; };

QString FileSystemEntry::relativePath() const { return m_relativePath; };

QString FileSystemEntry::name() const { return m_fileInfo.fileName(); };

QString FileSystemEntry::baseName() const { return m_fileInfo.baseName(); };

QString FileSystemEntry::parentDir() const { return m_fileInfo.absolutePath(); };

QString FileSystemEntry::suffix() const { return m_fileInfo.completeSuffix(); };

qint64 FileSystemEntry::size() const { return m_fileInfo.size(); };

bool FileSystemEntry::isDir() const { return m_fileInfo.isDir(); };
...
QHash<int, QByteArray> FileSystemModel::roleNames() const { return {{Qt::UserRole, "modelData"}}; }

QString FileSystemModel::path() const { return m_path; }
...
bool FileSystemModel::recursive() const { return m_recursive; }
...
bool FileSystemModel::watchChanges() const { return m_watchChanges; }
...
bool FileSystemModel::sortReverse() const { return m_sortReverse; }
...
FileSystemModel::Filter FileSystemModel::filter() const { return m_filter; }
...
QStringList FileSystemModel::nameFilters() const { return m_nameFilters; }
...
void FileSystemModel::updateEntriesForDir(const QString& dir) {
  ...
  future
      ...
      .onCanceled(this, [dir, this]() { m_futures.remove(dir); });
}

void FileSystemModel::applyChanges(const QSet<QString>& removedPaths,
  ...
  std::sort(
      ...
      [this](const FileSystemEntry* a, const FileSystemEntry* b) { return compareEntries(a, b); });
  ...
  for ( const auto& entry : std::as_const(newEntries) ) {
    ...
    if ( insertStart == -1 ) {
      ...
      beginInsertRows(QModelIndex(), insertStart,
