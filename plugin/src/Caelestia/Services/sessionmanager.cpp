...
SessionManager::SessionManager(QObject* parent) : QObject(parent) {
  ...
  bool ok = bus->connect(LOGIN_SERVICE, LOGIN_PATH, LOGIN_IFACE, "PrepareForSleep", this,
                         SLOT(handlePrepareForSleep(bool)));
  if ( !ok )
    qCWarning(lcSessionManager) << "Failed to connect to PrepareForSleep signal:"
                                << bus->lastError().message();

  auto sessionMsg =
      QDBusMessage::createMethodCall(LOGIN_SERVICE, LOGIN_PATH, LOGIN_IFACE, "GetSession");
  ...
  ok = bus->connect(LOGIN_SERVICE, m_sessionPath, SESSION_IFACE, "Lock", this,
                    SLOT(handleLockRequested()));
  if ( !ok )
    qCWarning(lcSessionManager) << "Failed to connect to Lock signal:"
                                << bus->lastError().message();

  ok = bus->connect(LOGIN_SERVICE, m_sessionPath, SESSION_IFACE, "Unlock", this,
                    SLOT(handleUnlockRequested()));
  if ( !ok )
    qCWarning(lcSessionManager) << "Failed to connect to Unlock signal:"
                                << bus->lastError().message();
}
...
bool SessionManager::exec(const QStringList& command) {
  ...
  if ( (cmd == u"systemctl"_s || cmd == u"loginctl"_s) && command.size() == 2 )
    cmd = command.at(1);
  if ( cmd == u"loginctl"_s && command.size() == 3 && command.at(1) == u"terminate-user"_s &&
       command.at(2).isEmpty() )
...
void SessionManager::logout() { callSession("Terminate"); }

void SessionManager::suspend() { callManager("Suspend"); }
...
void SessionManager::hibernate() {
  if ( queryHibernateAvailable() ) {
    ...
    if ( !toaster )
      return;
    toaster->toast(tr("Hibernate failed"), tr("Enable hibernation to use this feature."), "warning",
                   Toast::Type::Warning);
  }
}

void SessionManager::poweroff() { callManager("PowerOff"); }

void SessionManager::reboot() { callManager("Reboot"); }
...
bool SessionManager::queryHibernateAvailable() const {
  ...
  auto hibernateMsg =
      QDBusMessage::createMethodCall(LOGIN_SERVICE, LOGIN_PATH, LOGIN_IFACE, "CanHibernate");
  const QDBusReply<QString> hibernateReply = bus->call(hibernateMsg);
  if ( !hibernateReply.isValid() ) {
    qCWarning(lcSessionManager) << "Failed to query hibernate support:"
                                << hibernateReply.error().message();
  } else {
...
void SessionManager::call(const QString& path, const QString& iface, const QString& method,
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [method](QDBusPendingCallWatcher* self) {
            const QDBusPendingReply<> reply = *self;
            if ( reply.isError() )
              qCWarning(lcSessionManager)
                  << "Call to" << method << "failed:" << reply.error().message();
            self->deleteLater();
          });
}
...
void SessionManager::handleLockRequested() { emit lockRequested(); }

void SessionManager::handleUnlockRequested() { emit unlockRequested(); }
