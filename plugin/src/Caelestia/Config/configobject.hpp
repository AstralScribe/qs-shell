...
inline QVariantMap vmap(std::initializer_list<std::pair<QString, QVariant>> entries) {
  QVariantMap map;
  for ( const auto& [key, value] : entries ) map.insert(std::move(key), std::move(value));
  return map;
}
...
public:                                                                            \
  [[nodiscard]] Type name() const {                                                \
    if ( isOverlay() )                                                             \
      qCWarning(caelestia::config::lcConfig,                                       \
                "Reading global-only option '%s' on per-monitor overlay",          \
  ...
  void set_##name(const Type& val) {                                               \
    if ( isOverlay() )                                                             \
      qCWarning(caelestia::config::lcConfig,                                       \
                "Writing global-only option '%s' on per-monitor overlay",          \
                qUtf8Printable(propertyPath(QStringLiteral(#name))));              \
    if ( caelestia::config::ConfigObject::updateMember(m_##name, val) ) {          \
      markPropertyLoaded(QStringLiteral(#name));                                   \
      Q_EMIT name##Changed();                                                      \
...
public:
  ...
  template <typename T>
  static bool updateMember(T& member, const T& value) {
