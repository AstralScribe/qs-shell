...
void ConfigObject::loadFromJson(const QJsonObject& obj) {
  ...
  for ( int i = basePropertyOffset(); i < meta->propertyCount(); ++i ) {
    ...
    if ( prop.metaType().id() == QMetaType::QStringList ) {
      ...
      for ( const auto& v : jsonArr ) list.append(v.toString());
...
QJsonObject ConfigObject::toJsonObject() const {
  ...
  for ( int i = basePropertyOffset(); i < meta->propertyCount(); ++i ) {
    ...
    if ( prop.metaType().id() == QMetaType::QStringList ) {
      ...
      for ( const auto& s : strList ) arr.append(s);
      obj.insert(key, arr);
      continue;
    }
...
void ConfigObject::syncFromGlobal(ConfigObject* global) {
  ...
  const auto* meta = metaObject();
  qCDebug(lcConfig) << "Syncing" << meta->className()
                    << "from global, loaded keys:" << m_loadedKeys;
...
int ConfigObject::basePropertyOffset() { return ConfigObject::staticMetaObject.propertyCount(); }
...
bool ConfigObject::isOverlay() const { return m_global != nullptr; }

bool ConfigObject::isGlobalOnly(const QString& name) const {
  return isOverlay() && m_globalOnlyKeys.contains(name);
}

void ConfigObject::markPropertyLoaded(const QString& name) { m_loadedKeys.insert(name); }
...
void ConfigObject::markGlobalOnly(const QString& name) { m_globalOnlyKeys.insert(name); }
