...
RootConfig::RootConfig(QObject* parent) : ConfigObject(parent) { }

bool RootConfig::recentlySaved() const { return m_recentlySaved; }
...
void RootConfig::setupFileBackend(const QString& path, const QString& screen) {
  ...
  connect(m_cooldownTimer, &QTimer::timeout, this, [this] { m_recentlySaved = false; });
  ...
  QTimer::singleShot(0, this, [this, result] { emitLoadSignals(result, false); });
}
...
void RootConfig::onWatcherEvent() {
  ...
  // Only reload when the file actually changed (directory is watched so events fire for unrelated
  // files)
...
std::optional<QString> RootConfig::reloadFromFile() {
  ...
  if ( error.error != QJsonParseError::NoError ) {
    ...
    qCWarning(lcConfig, "Failed to parse %s: %s", qUtf8Printable(m_filePath),
...
void RootConfig::save() { saveToFile(); }

void RootConfig::emitLoadSignals(const std::optional<QString>& result, bool emitLoaded) {
  if ( !result.has_value() )
    return;

  for ( const auto& key : std::as_const(m_lastUnknownKeys) ) emit unknownOption(key, m_screen);
...
void RootConfig::reload() { emitLoadSignals(reloadFromFile()); }
