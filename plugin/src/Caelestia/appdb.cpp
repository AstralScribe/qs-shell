...
AppEntry::AppEntry(QObject* entry, unsigned int frequency, QObject* parent)
  ...
  for ( const auto& prop : {"name", "comment", "execString", "startupClass", "genericName",
                            "categories", "keywords"} ) {
...
AppDb::AppDb(QObject* parent)
    : QObject(parent), m_timer(new QTimer(this)), m_uuid(QUuid::createUuid().toString()) {
...
QStringList AppDb::favouriteApps() const { return m_favouriteApps; }
