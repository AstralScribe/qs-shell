...
QString ImageAnalyser::source() const { return m_source; }
...
QQuickItem* ImageAnalyser::sourceItem() const { return m_sourceItem; }
...
int ImageAnalyser::rescaleSize() const { return m_rescaleSize; }
...
QColor ImageAnalyser::dominantColour() const { return m_dominantColour; }

qreal ImageAnalyser::luminance() const { return m_luminance; }
...
void ImageAnalyser::requestUpdate() {
  ...
  if ( !m_sourceItem || (m_sourceItem->window() && m_sourceItem->window()->isVisible() &&
    ...
    if ( !m_sourceItem->window() ) {
      QObject::connect(m_sourceItem, &QQuickItem::windowChanged, this,
                       &ImageAnalyser::requestUpdate, Qt::SingleShotConnection);
    } else if ( !m_sourceItem->window()->isVisible() ) {
      QObject::connect(m_sourceItem->window(), &QQuickWindow::visibleChanged, this,
                       &ImageAnalyser::requestUpdate, Qt::SingleShotConnection);
    }
    if ( m_sourceItem->width() <= 0 ) {
      QObject::connect(m_sourceItem, &QQuickItem::widthChanged, this, &ImageAnalyser::requestUpdate,
                       Qt::SingleShotConnection);
    }
    if ( m_sourceItem->height() <= 0 ) {
      QObject::connect(m_sourceItem, &QQuickItem::heightChanged, this,
                       &ImageAnalyser::requestUpdate, Qt::SingleShotConnection);
    }
  }
}
  ...
  if ( m_sourceItem ) {
    ...
    QObject::connect(grabResult.data(), &QQuickItemGrabResult::ready, this, [grabResult, this]() {
      m_futureWatcher->setFuture(
          QtConcurrent::run(&ImageAnalyser::analyse, grabResult->image(), m_rescaleSize));
    });
  } else {
...
void ImageAnalyser::analyse(QPromise<AnalyseResult>& promise, const QImage& image,
                            int rescaleSize) {
  if ( image.isNull() ) {
  ...
  promise.addResult(
      qMakePair(QColor((0xFFu << 24) | dominantColour), count == 0 ? 0.0 : totalLuminance / count));
}
