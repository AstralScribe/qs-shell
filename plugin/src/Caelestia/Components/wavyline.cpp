...
int WavyLine::lineWidth() const { return m_lineWidth; }
...
qreal WavyLine::amplitudeMultiplier() const { return m_amplitudeMultiplier; }
...
int WavyLine::frequency() const { return m_frequency; }
...
qreal WavyLine::startX() const { return m_startX; }
...
qreal WavyLine::fullLength() const { return m_fullLength; }
...
QColor WavyLine::color() const { return m_color; }
...
qreal WavyLine::waveProgress() const { return m_waveProgress; }
...
WavyLine::PathType WavyLine::pathType() const { return m_pathType; }
...
qreal WavyLine::startAngle() const { return m_startAngle; }
...
qreal WavyLine::fullAngle() const { return m_fullAngle; }
...
qreal WavyLine::radius() const { return m_radius; }
...
qreal WavyLine::value() const { return m_value; }
...
void WavyLine::paintArc(QPainter* painter) {
  ...
  const auto radius =
      m_radius > 0 ? m_radius : (qMin(width(), height()) - m_lineWidth - 2 * amplitude) / 2.0;
