...
void CUtils::saveItem(QQuickItem* target, const QUrl& path, const QRect& rect, QJSValue onSaved,
  ...
  if ( rect.isValid() && !qFuzzyCompare(scale + 1.0, 2.0) ) {
    scaledRect =
        QRectF(rect.left() * scale, rect.top() * scale, rect.width() * scale, rect.height() * scale)
            .toRect();
  }
...
bool CUtils::copyFile(const QUrl& source, const QUrl& target, bool overwrite) {
  ...
  if ( overwrite && QFile::exists(target.toLocalFile()) ) {
    if ( !QFile::remove(target.toLocalFile()) ) {
      qCWarning(lcCUtils) << "copyFile: overwrite was specified but failed to remove"
                          << target.toLocalFile();
      return false;
    }
  }
...
qreal CUtils::clamp(qreal value, qreal min, qreal max) { return qBound(min, value, max); }

namespace {

// DFS over the visual item tree (childItems), returning the first descendant matching the
// predicate. Unlike QObject::findChild, this walks parentItem/childItems relationships so it
// traverses the QML visual hierarchy.
...
template <typename Predicate>
void findChildrenDfs(QQuickItem* root, Predicate&& match, QList<QQuickItem*>& out) {
  const auto children = root->childItems();
  for ( QQuickItem* const child : children ) {
    if ( match(child) ) {
      out.append(child);
...
QQuickItem* CUtils::findChild(QQuickItem* root, const QString& name) {
  ...
  return findChildDfs(root, [&name](const QQuickItem* item) { return item->objectName() == name; });
}
...
QList<QQuickItem*> CUtils::findChildren(QQuickItem* root, const QString& name) {
  ...
  if ( root ) {
    findChildrenDfs(
        root, [&name](const QQuickItem* item) { return item->objectName() == name; }, children);
  }
  return children;
}
...
QList<QQuickItem*> CUtils::findChildrenMatching(QQuickItem* root, const QString& pattern) {
  ...
  if ( root ) {
    ...
    findChildrenDfs(
        root, [&re](const QQuickItem* item) { return re.match(item->objectName()).hasMatch(); },
        children);
  }
  return children;
}
...
QString CUtils::version() const { return QStringLiteral(CAELESTIA_VERSION); }

QString CUtils::qtVersion() const { return QStringLiteral(QT_VERSION_STR); }
